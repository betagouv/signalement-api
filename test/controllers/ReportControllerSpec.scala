...
class ReportControllerSpec(implicit ee: ExecutionEnv) extends Specification with Results with Mockito {
  ...
  trait Context extends Scope {
    ...
    mockReportRepository.create(any[Report]) answers { (report: Report) => Future(report.asInstanceOf[Report]) }
    mockReportRepository.update(any[Report]) answers { (report: Report) => Future(report.asInstanceOf[Report]) }
    ...
    mockUserRepository.create(any[User]) answers { (user: User) => Future(user.asInstanceOf[User]) }

    mockEventRepository.createEvent(any[Event]) answers { (event: Event) => Future(event.asInstanceOf[Event]) }

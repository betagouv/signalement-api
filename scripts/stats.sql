...
select count(1) "Nb signalements des 30 derniers jours",
       ...
       ((count(*) filter ( where website_url is not null and company_id is null and company_country is not null))::numeric / (count(*) filter ( where website_url is not null and company_id is null))::numeric * 100)::numeric(5,2) "% sign entreprise etranger parmi les sign internets non ident"
from reports

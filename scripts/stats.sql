...
-- Nb utilisateurs DGCCRF connectés au mois de mai
select count(distinct(u.id)) "Nb utilisateurs DGCCRF connectés au mois de mai"
...
-- Nb d'entreprises pour lesquelles un contrôle DGCCRF a été déclaré au mois de mai
select count(distinct company_id) "Nb d''entreprises controlées au mois de mai"
from events
where action = 'Contrôle effectué'
and creation_date >= '2021-05-01'::timestamp
and creation_date <= '2021-06-01'::timestamp;
...
-- % signalements lus (signalements du mois de mai)
...
(count(*) filter ( where status not in ('NA', 'Lanceur d''alerte') ))::numeric * 100)::numeric(5,2) "Signalements du mois de mai : % lus"
from reports
where creation_date >= '2021-05-01'::timestamp
and creation_date <= '2021-06-01'::timestamp;

-- % signalements avec une réponse (Signalements du mois de mai)
select ((count(*) filter ( where status in ('Promesse action', 'Signalement infondé', 'Signalement mal attribué') ))::numeric /
(count(*) filter ( where status not in ('NA', 'Lanceur d''alerte') ))::numeric * 100)::numeric(5,2) "Signalements du mois de mai : % avec une réponse"
from reports
where creation_date >= '2021-05-01'::timestamp
and creation_date <= '2021-06-01'::timestamp;

-- temps de lecture moyen des signalements (signalements du mois de mai)
select (percentile_cont(0.5) within group ( order by read_delay) / (60 * 24))::numeric(6,2) "Signalements du mois de mai : temps de lecture moyen"
from report_data d, reports r
where r.id = d.report_id
and creation_date >= '2021-05-01'::timestamp
and creation_date <= '2021-06-01'::timestamp;

-- répartition des réponses (réponses du mois de mai)
select count(1) "Nb réponses du mois de mai",
...
-- avis sur la réponse du Pro (avis du mois de mai)
select count(1) "Nb avis du mois de mai sur la réponse Pro",
...
select count(1) "Nb avis du mois de mai sur les pages d'info",

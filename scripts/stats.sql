...
-- Nb utilisateurs DGCCRF connectés au mois de septembre
select count(distinct(u.id)) "Nb utilisateurs DGCCRF connectés au mois de septembre"
from users u, auth_attempts a
where u.role = 'DGCCRF'
and u.email = a.login
and timestamp >= '2021-09-01'::timestamp
and timestamp <= '2021-10-01'::timestamp;
...
-- Nb d'entreprises pour lesquelles un contrôle DGCCRF a été déclaré au mois de septembre
select count(distinct company_id) "Nb d''entreprises controlées au mois de septembre"
from events
where action = 'Contrôle effectué'
and creation_date >= '2021-09-01'::timestamp
and creation_date <= '2021-10-01'::timestamp;
...
-- % signalements lus (signalements du mois de septembre)
...
(count(*) filter ( where status not in ('NA', 'Lanceur d''alerte') ))::numeric * 100)::numeric(5,2) "Signalements du mois de septembre : % lus"
from reports
where reports.creation_date > '2021-10-01'::timestamp - '81 days'::interval;

-- % signalements avec une réponse (Signalements du mois de septembre)
select ((count(*) filter ( where status in ('Promesse action', 'Signalement infondé', 'Signalement mal attribué') ))::numeric /
(count(*) filter ( where status not in ('NA', 'Lanceur d''alerte') ))::numeric * 100)::numeric(5,2) "Signalements du mois de septembre : % avec une réponse"
from reports
where creation_date > '2021-10-01'::timestamp - '81 days'::interval;


-- temps de lecture moyen des signalements (signalements du mois de septembre)
...
where reports.id = events.report_id
  and action = 'Première consultation du signalement par le professionnel'
    and reports.creation_date >= '2021-09-01'::timestamp
    and reports.creation_date <= '2021-10-01'::timestamp;

select (percentile_cont(0.5) within group ( order by read_delay) / (60 * 24))::numeric(6,2) "Signalements du mois de septembre : temps de lecture moyen"
from read_delay_table;


-- répartition des réponses (réponses du mois de septembre)
select count(1) "Nb réponses du mois de septembre",
...
-- avis sur la réponse du Pro (avis du mois de septembre)
select count(1) "Nb avis du mois de septembre sur la réponse Pro",
...
select count(1) "Nb avis du mois de septembre sur les pages d'info",
...
from ratings
where creation_date >= '2021-09-01'::timestamp
and creation_date <= '2021-10-01'::timestamp;

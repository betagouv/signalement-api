...
class UserReportsFiltersController(
  ...
  def save(): Action[JsValue] =
    SecuredAction(WithPermission(UserPermission.crudUserReportsFilters)).async(parse.json) { implicit request =>
      request.body
  ...
  def get(name: String): Action[AnyContent] =
    SecuredAction(WithPermission(UserPermission.crudUserReportsFilters)).async { implicit request =>
      userReportsFiltersOrchestrator

...
class ReportListController @Inject()(reportRepository: ReportRepository,
  ...
  def handleRelance() = SecuredAction.async { implicit request =>
    logger.debug(s"handleRelance")
    ...
    logger.debug("last21Days " + last21Days)

    reportRepository.getReportsForStatusWithUser(SIGNALEMENT_TRANSMIS).flatMap( reports => {
      reports.map( tuple => {
        val report = tuple._1
        val user = tuple._2
        ...
        eventRepository.getEvents(report.id.get, EventFilter(None, Some(RELANCE))).flatMap(events => {
          ...
          events.length match {
            case aux if aux > 2 => {
              if (events.head.creationDate.get.toLocalDateTime.isBefore(lastWeek)) {
                val event  = Event(
                    ...
                    report.id,
                    UUID.fromString(ziggyUuid),
                    Some(OffsetDateTime.now()),
                    Constants.EventType.PRO,
                    Constants.ActionEvent.CONSULTE_IGNORE,
                    None,
                    Some(s"Clôture automatique après 2 fois 7 jours et mise à Signalement consulté ignoré")
                )

                logger.debug(s"- report id=${report.id.get} siret=${report.companySiret.get} date=${report.creationDate.get}")
            ...
            case _ => eventRepository.getEvents(report.id.get, EventFilter(None, Some(CONTACT_EMAIL))).flatMap(events => {
              if (events.head.creationDate.get.toLocalDateTime.isBefore(lastWeek)) {
                val event = Event(
                  Some(UUID.randomUUID()),
                  report.id,

...
import utils.Constants.StatusPro.{A_TRAITER, PROMESSE_ACTION, SIGNALEMENT_INFONDE, SIGNALEMENT_REFUSE, SIGNALEMENT_TRANSMIS, TRAITEMENT_EN_COURS}
...
class StatisticController @Inject()(reportRepository: ReportRepository,
  ...
  def getStatistics = UserAwareAction.async { implicit request =>
    ...
    for {
      ...
      reportsCountSendedToPro <- reportRepository.nbSignalementsBetweenDates(departments = Some(Departments.AUTHORIZED), statusList = Some(List(SIGNALEMENT_TRANSMIS, PROMESSE_ACTION, SIGNALEMENT_INFONDE, SIGNALEMENT_REFUSE)))
      reportsCountSendedToProBase <- reportRepository.nbSignalementsBetweenDates(departments = Some(Departments.AUTHORIZED), statusList = Some(List(SIGNALEMENT_TRANSMIS, PROMESSE_ACTION, SIGNALEMENT_INFONDE, SIGNALEMENT_REFUSE, TRAITEMENT_EN_COURS, A_TRAITER)))
      ...
      reportsCountPromiseBase <- reportRepository.nbSignalementsBetweenDates(departments = Some(Departments.AUTHORIZED), statusList = Some(List(SIGNALEMENT_TRANSMIS, PROMESSE_ACTION, SIGNALEMENT_INFONDE)))

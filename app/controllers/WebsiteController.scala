...
class WebsiteController @Inject() (
  ...
  def fetchUnregisteredHost(q: Option[String], start: Option[String], end: Option[String]) =
    SecuredAction(WithRole(UserRoles.Admin, UserRoles.DGCCRF)).async { implicit request =>
      reportRepository
        .getWebsiteReportsWithoutCompany(DateUtils.parseDate(start), DateUtils.parseDate(end))
        .map(reports =>
          Ok(
            Json.toJson(
              reports
                ...
                .collect {
                  case (Some(host), reports) if q.map(host.contains(_)).getOrElse(true) => (host, reports.length)
                }

...
class SubscriptionController(
  ...
  def createSubscription =
    SecuredAction
      .andThen(WithRole(UserRole.AdminsAndReadOnlyAndAgents))
  ...
  def removeSubscription(uuid: UUID) =
    SecuredAction.andThen(WithRole(UserRole.AdminsAndReadOnlyAndAgents)).andThen(ForbidImpersonation).async {
      implicit request =>
        subscriptionOrchestrator

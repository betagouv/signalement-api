...
object EmailsExamplesUtils {
  ...
  def genCompany = Company(
    id = UUID.randomUUID,
    siret = SIRET.fromUnsafe("42405982200256"),
    creationDate = OffsetDateTime.now(),
    name = "CDISCOUNT- CDISCOUNT.COM- CDISCOUNT PRO - 3W REGIE - C LE MARCHE - C LES DEALS - OCEANIC - CONTINEN",
    address = Address(
      number = Some("120"),
      street = Some("QUAI DE BACALAN"),
      addressSupplement = Some("120-126"),
      postalCode = Some("33300"),
      city = Some("BORDEAUX"),
      country = None
    ),
    activityCode = Some("47.91A"),
    isHeadOffice = true,
    isOpen = true,
    isPublic = true,
    brand = Some("CDISCOUNT- CDISCOUNT.COM- CDISCOUNT - PRO - 3W REGIE   C LE  MARCHE - C L - ES DEALS"),

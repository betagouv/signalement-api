...
class CompanyRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, val userRepository: UserRepository)(
  ...
  class CompanyTable(tag: Tag) extends Table[Company](tag, "companies") {
    ...
    def constructCompany: CompanyData => Company = {
      case (id, siret, creationDate, name, address, postalCode, _, activityCode) =>
        Company(id, siret, creationDate, name, address, postalCode, activityCode)
    }

    def extractCompany: PartialFunction[Company, CompanyData] = {
      case Company(id, siret, creationDate, name, address, postalCode, activityCode) =>
        (
          id,
          siret,
          creationDate,
          ...
          address,
          postalCode,
          postalCode.map(Departments.fromPostalCode(_)).flatten,
          activityCode
        )
    }

...
class CompanyActivationOrchestrator(
    val companyRepository: CompanyRepositoryInterface,
    val accessTokenRepository: AccessTokenRepositoryInterface,
    val userAccessOrchestrator: AccessesOrchestrator
  ...
  def sendActivationLink(siret: SIRET, activationLinkRequest: ActivationLinkRequest): Future[Unit] =
    for {
      ...
      _ <- userAccessOrchestrator.addUserOrInvite(company, activationLinkRequest.email, AccessLevel.ADMIN, None)
    } yield ()

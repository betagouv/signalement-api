               ...
               eventRepository
                 .createEvent(
                   Event(
                     ...
                     stringToDetailsJsValue(s"Entreprise précédente : Siret ${report.companySiret
                       .getOrElse("non renseigné")} - ${report.companyAddress.getOrElse("Adresse non renseignée")}")
                   )
                 )
                 .map(Some(_))
             case _ => Future(None)

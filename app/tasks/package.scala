...
package object tasks {
  ...
  def computeStartingTime(startTime: LocalTime): FiniteDuration = {
    ...
    val startDate: LocalDateTime =
      if (LocalTime.now().truncatedTo(ChronoUnit.MILLIS).isAfter(startTime))
        LocalDate.now().plusDays(1).atTime(startTime)
      else LocalDate.now().atTime(startTime)

    (LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS).until(startDate, ChronoUnit.SECONDS) % (24 * 7 * 3600)).seconds
  }

        ...
        reports.entities
          .filter(report =>
            subscription._1.departments.isEmpty || subscription._1.departments
              .map(Some(_))
              .contains(report.companyPostalCode.flatMap(Departments.fromPostalCode(_)))
          )
          .filter(report =>
          ...
          .filter(report =>
            subscription._1.countries.isEmpty || subscription._1.countries.map(Some(_)).contains(report.companyCountry)
          )
